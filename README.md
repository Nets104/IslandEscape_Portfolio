...
- 인벤토리 및 퀵슬롯 시스템 구현
- 월드 아이템 획득/드롭 로직 구현
- 나무, 바위, 금속, 덩굴 채집 및 바닷물 채집 상호작용 구현
...
### Gathering / Foliage Interaction

- `Source/IslandEscape/Public/IslandEscapeCharacter.h`
...
플레이어 캐릭터의 F키 상호작용, 도끼 타격, 해변 바닷물 채집, 인스턴스 폴리지 기반 자원 채집 흐름을 포함합니다. 덩굴은 `FindBestVineHit`에서 조준 보정과 인스턴스 단위 차폐 검사를 함께 처리해, 화면에 표시된 덩굴과 실제로 사라지는 덩굴이 어긋나지 않도록 했습니다.
...
- 덩굴 채집 힌트와 실제 채집 대상이 서로 다른 인스턴스를 가리킬 수 있는 문제

## Build Download

패키징된 실행 파일은 용량 문제로 레포에 직접 커밋하지 않고 Google Drive 링크로 공유합니다.

- Windows Build v1.1: [Google Drive에서 다운로드](https://drive.google.com/file/d/1iuHvN7x2qsqIwXokShcomtpt8mYfC_sh/view?usp=drive_link)
